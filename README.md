...
flowchart LR
    ...
    subgraph STEP1 ["STEP 1 · 데이터 규격화"]
        direction TB
        A1[PDF/DOCX 파싱 → LLM 청킹]:::s1
        A2[정규화 — 노이즈 제거 · 짧은 청크 병합]:::s1
        A3{content_hash<br>중복 확인}:::s1
        A4[Gemini Embedding 2<br>1536차원 벡터 변환]:::s1
        
        A1 --> A2 --> A3
        A3 --신규--> A4
    end

    DB1[(DB 저장: doc_chunks)]:::db

    subgraph STEP2 ["STEP 2 · 계층 태깅"]
        direction TB
        B1[단계 1 — H1/H2/H3 Master<br>+ domain_profile 생성]:::s2
        B3[단계 2 — 청크별 계층 일괄 적용]:::s2
        
        B1 --> B3
    end

    DB2[(DB 저장: metadata)]:::db

    subgraph STEP3 ["STEP 3 · QA 생성"]
        direction TB
        C1[H1/H2 필터 — 계층 기반 청크 선별]:::s3
        C2[domain_profile 조회<br>doc_metadata 캐시 우선]:::s3
        C3[병렬 QA 생성 — 다중 모델 동시 생성]:::s3
        
        C1 --> C2 --> C3
    end

    DB3[(DB 저장: qa_gen_results)]:::db

    subgraph STEP4 ["STEP 4 · QA 평가"]
        direction TB
        D1[Layer 1-A Syntax<br>필드 · 길이 검사]:::s4
        D2[Layer 1-B Statistics<br>다양성 · 중복률]:::s4
        D3[Layer 2 RAG Triad<br>관련성 · 근거성 · 맥락성]:::s4
        D4[Layer 3 Quality<br>완전성]:::s4
        D5[최종 점수 집계<br>구문·통계 ×0.05 / RAG ×0.65 / 품질 ×0.25]:::s4
        
        D1 & D2 & D3 & D4 --> D5
    end

    DB4[(DB 저장: qa_eval_results)]:::db

    subgraph STEP5 ["STEP 5 · 결과 확인"]
        direction TB
        E1[평가 결과 확인<br>QA 상세 · 레이어별 점수]:::s5
        E2[리포트 내보내기<br>XLSX / HTML / ZIP]:::s5
        E3[대시보드<br>집계 지표 · 점수 추이 · 등급 분포]:::s5
    end
