...
flowchart TD
    ...
    subgraph S1["STEP 1. 데이터 규격화"]
        B[PyMuPDF 파싱\nSection-First 청킹] --> C[normalize_text\n특수문자·줄바꿈·짧은 청크 정리]
        C --> D{content_hash\n중복 확인}
        D -- 신규 --> E[Gemini Embedding 2\n1536dim 벡터화]
        D -- 중복 --> X1([skip])
        E --> F[(Supabase\ndoc_chunks)]
    end
    ...
    subgraph S2["STEP 2. 계층 태깅 — 3-Pass Master"]
        G[Pass 1: analyze-hierarchy\ndoc_chunks 샘플 → L1 master 확정] --> H
        H[Pass 2: analyze-l2-l3\nL1 기반 → L2/L3 master 동시 생성] --> I
        I[Pass 3: apply-granular-tagging\n청크별 master 에서 선택만\n신규 생성 금지] --> J[(doc_chunks.metadata\nhierarchy_l1/l2/l3 업데이트)]
    end

    J --> K

    subgraph S3["STEP 3. QA 생성"]
        K[L1/L2 필터 → doc_chunks 조회\nheading·colophon skip] --> L
        L[domain_profiler\ndoc_chunks 샘플 → domain_profile\njob당 1회] --> M
        M[적응형 프롬프트 빌드\nXML 태그 구조] --> N
        N[ThreadPoolExecutor\n병렬 생성\n4~8개 유연 조정] --> O[(qa_gen_results)]
    end
    ...
    subgraph S4["STEP 4. 4레이어 평가"]
        P[Layer 1-A\nSyntax Validation] --> Q
        Q[Layer 1-B\nDataset Statistics\n다양성·중복률] --> R
        R[Layer 2\nRAG Triad\nRelevance · Groundedness · Clarity] --> T
        T[Layer 3\nQuality Score\nFactuality · Completeness · Groundedness] --> U
        U[최종 점수 집계\nsyntax×0.2 + stats×0.2\n+ rag×0.3 + quality×0.3] --> V[(qa_eval_results\nfinal_score · final_grade)]
    end

    V --> W

    subgraph S5["STEP 5. 대시보드"]
        W[GET /api/dashboard/metrics\nSupabase 집계] --> Z[총 QA · 평균 점수 · 등급 분포\n최근 실행 이력 · 점수 추이]
    end
```
