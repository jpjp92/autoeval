...
```
┌──────────────────────────────────────────────────────────────┐
│  STEP 1. 데이터 규격화 (Data Standardization)                 │
│                                                              │
│  PDF/DOCX 업로드                                             │
│    └─ PyMuPDF 파싱 → Section-First 청킹                      │
│    └─ normalize_text (특수문자 정규화, 줄바꿈 결합, 짧은 청크 병합) │
│    └─ content_hash 기반 중복 방지                             │
│    └─ Gemini Embedding 2 (1536dim) 벡터화                    │
│    └─ Supabase doc_chunks 저장                               │
└──────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌──────────────────────────────────────────────────────────────┐
│  STEP 2. 계층 태깅 — 3-Pass Master 방식 (Hierarchy Tagging)  │
│                                                              │
│  Pass 1: analyze-hierarchy                                   │
│    └─ doc_chunks 샘플 → LLM → L1 master (3~5개) 확정         │
│                                                              │
│  Pass 2: analyze-l2-l3                                       │
│    └─ L1 master 기반 → LLM 1회 → L2/L3 master 동시 생성       │
│                                                              │
│  Pass 3: apply-granular-tagging                              │
│    └─ 청크별 master 목록에서 선택만 (신규 생성 금지)             │
│    └─ doc_chunks.metadata.hierarchy_l1/l2/l3 업데이트         │
│                                                              │
│  → 프론트엔드 L1/L2 드롭다운으로 생성 범위 지정 가능             │
└──────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌──────────────────────────────────────────────────────────────┐
│  STEP 3. QA 생성 (Generation)                                │
│                                                              │
│  L1/L2 필터 → doc_chunks 직접 조회                           │
│    └─ heading/colophon 청크 skip                             │
│    └─ doc_chunks 샘플 → LLM → domain_profile (job당 1회)     │
...
│    - 의도 유형: 근거 있는 유형만 선택, 동일 유형 최대 2회        │
│    - 다양성: 우선 그룹(factoid/definition/how) ≤ 50%          │
│    - procedure 유형: 문서에 순서 있는 단계(1→2→3) 명시 시에만   │
│    - 질문 근거: 컨텍스트에 명시된 사실/정의/절차에만 한정         │
│    - 답변 스타일: 메타 표현 시작 금지 ("컨텍스트에 따르면" 등)    │
└──────────────────────────────────────────────────────────────┘
                            │
                            ▼
┌──────────────────────────────────────────────────────────────┐
│  STEP 4. 4레이어 평가 (Evaluation)                           │
│                                                              │
...
│                                                              │
│  /api/dashboard/metrics → Supabase 집계                      │
│    └─ 총 QA 수, 평균 점수, 문서 수, 통과율                     │
│    └─ 최근 파이프라인 실행 이력 (10건)                         │
│    └─ 등급 분포 (A+/A/B+/B/C/F)                              │
│    └─ 점수 추이 (score_trend)                                │
└──────────────────────────────────────────────────────────────┘
