            ...
            {open && (
              <div className="absolute right-0 top-full mt-2 w-80 z-[9999] bg-white dark:bg-slate-800 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl shadow-slate-300/50 dark:shadow-black/80 overflow-hidden">
                ...
                <div className="px-4 py-2.5 border-t border-slate-100 dark:border-white/8 flex items-center justify-between">
                  <span className="text-xs text-slate-400 dark:text-slate-500">테마</span>
                  <button
                    onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
                    className="flex items-center gap-2 px-2.5 py-1 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
                    title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}
                  >
                    {theme === 'dark'
                      ? <><Sun className="w-3.5 h-3.5" /> 라이트 모드</>
                      : <><Moon className="w-3.5 h-3.5" /> 다크 모드</>
                    }
                  </button>
                </div>
              </div>
