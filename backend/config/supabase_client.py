...
async def get_dashboard_metrics() -> Dict[str, Any]:
    ...
    try:
        # 1) qa_gen_results 전체 조회 (summary + recent)
        gen_res = supabase_client.table("qa_gen_results").select(
            "id, job_id, source_doc, metadata, stats, created_at"
        ).order("created_at", desc=True).execute()
        gen_rows = gen_res.data or []

        # 2) qa_eval_results 전체 조회
        eval_res = supabase_client.table("qa_eval_results").select(
        ...
        chunks_res = supabase_client.table("doc_chunks").select("metadata").execute()
